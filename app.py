...
def water_effect(decode_array_to_img):
    ...
    status, output_img = cv2.imencode('.PNG', water_img)

    return output_img

def negative_effect(decode_array_to_img):
    negative_img = cv2.bitwise_not(decode_array_to_img)
    status, output_img = cv2.imencode('.PNG', negative_img)

    return output_img
def hdr_effect(decode_array_to_img):
    hdr_img = cv2.detailEnhance(decode_array_to_img)
